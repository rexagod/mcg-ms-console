export const app = {
  ...
  waitForLoad: (timeout: number = 160000) => {
    cy.log('checking if any component is in loading state or not');
    cy.get('.co-m-loader', { timeout }).should('not.exist'); // eslint-disable-line cypress/require-data-selectors
    cy.get('.pf-c-spinner', { timeout }).should('not.exist'); // eslint-disable-line cypress/require-data-selectors
    cy.get('.skeleton-catalog--grid', { timeout }).should('not.exist'); // eslint-disable-line cypress/require-data-selectors
    cy.get('.loading-skeleton--table', { timeout }).should('not.exist'); // eslint-disable-line cypress/require-data-selectors
