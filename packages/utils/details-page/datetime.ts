...
export const fromNow = (dateTime: string | Date, now?: Date, options?) => {
  ...
  const justNow = i18n.t('plugin__mcg-ms-console~Just now');
  ...
  if (options?.omitSuffix) {
    if (days) {
      return i18n.t('plugin__mcg-ms-console~{{count}} day', { count: days });
    }
    if (hours) {
      return i18n.t('plugin__mcg-ms-console~{{count}} hour', { count: hours });
    }
    return i18n.t('plugin__mcg-ms-console~{{count}} minute', { count: minutes });
  }
